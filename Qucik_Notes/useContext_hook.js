...
1. Prop drilling means passing props directly from parent to child only. F.
2. The useContext() hook helps avoid prop drilling. T.
3. Context must be created with `React.createContext()`. T
4. You can only provide a string value in context. F
5. The `Provider` component wraps children components to give them access to context. T
6. Every component in the tree automatically consumes context without using useContext(). F
7. The value passed to `Provider` can be any JavaScript data type. T
8. Context values can be updated using useState inside the Provider. T
9. You cannot consume context in class components. F
10. `useContext()` hook can only be used inside functional components. T
...
7. What happens when the context value changes? high level note.
...
If you want, I can also provide answers or explanations for these! Would you like that?
